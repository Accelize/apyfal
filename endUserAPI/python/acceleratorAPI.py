...
#===================================
    ...
    def wait_instance_ready(self):
        try:
            ...
            if not ping( self.instance_url, 1, 72, 5 ): # 6 minutes timeout
                logger.error("Timed out while waiting CSP instance to boot.")
                return False
            return True
        except:
...
#===================================
    def __new__(cls, config_file, **kwargs):
        ...
        if provider.lower() == 'aws':
            return AWSClass(config_parser, **kwargs)
        elif provider.lower() == 'ovh':
            return OpenStackClass(config_parser, **kwargs)
        else:
...
#===================================
    ...
    def configure_accelerator(self, datafile=None, accelerator_parameters=None, **kwargs):
        try :
            ...
            if not ping(self.accelerator.getUrl(), 10):
